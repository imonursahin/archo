...
**Fixed**
- Terminals no longer resume each other's Claude conversation after the app is restarted. Archo used to work out which conversation a terminal owned by watching the transcripts folder and taking the most recently written file — but every Claude session running in that folder keeps its file fresh, so a terminal could adopt a sibling's conversation, or find nothing within the 30-second deadline and end up with no id at all. Those terminals then restarted with `claude --continue`, which hands the single newest conversation to every terminal that asks. Archo now pins the id up front with `--session-id` when it launches Claude, so a terminal owns its conversation from the first frame. A `claude` you type yourself gets the same treatment: the line is pinned as you press Enter, while it is still in the shell's editor. Only a command Archo could not rewrite (recalled from history, say) still falls back to watching the folder, and that watch now declines to guess while a sibling terminal in the same folder is waiting too. Terminals that never captured an id adopt an unclaimed conversation from their own folder, and a pinned conversation that was never actually started opens under its own id instead of failing to resume.
